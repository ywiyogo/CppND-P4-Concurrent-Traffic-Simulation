...
void Graphics::loadBackgroundImg()
{
    ...
    // Resize simulation windows for local work
    cv::resizeWindow(_windowName, cv::Size{background.cols/4, background.rows/4});
...
void Graphics::drawTrafficObjects()
{
    ...
    for (auto it : _trafficObjects)
    {
        ...
        if (it->getType() == ObjectType::objectIntersection)
        {
            ...
            cv::putText(_images.at(1), std::to_string(it->getID()),
                        cv::Point2d(posx, posy),        // Coordinates
